...
:: Run the admxcodegen command
echo Processing %filename%.admx...
admxcodegen "%filename%" "%input_file%" "%output_base%" --generate-csproj "%filename%Test" --generate-linqpad "%filename%LinqpadTest" --generate-buildlog

if !errorlevel! neq 0 (
    echo Error: admxcodegen command failed with error code !errorlevel!
    pause
    exit /b !errorlevel!
)
